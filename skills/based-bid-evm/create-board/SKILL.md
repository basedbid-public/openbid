...
```bash
npx ts-node src/evm/create-board.ts
```
...
The `createBoard` function accepts a `CreateEvmBoardSdk` type (inferred from `createEvmBoardSchema`):
...
//   banner: z.string().optional(), // file path to banner image
//   fees: boardFeesSchema.optional(),
// })
```
...
```typescript
import { createBoard } from './src/scripts/evm/create-board';
