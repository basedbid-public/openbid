# Claim Flash Token Fees (Solana) Skill

## Description

Claim accumulated trading fees from a **Flash Token** on **Solana**. This skill calls the BasedBid API at `/sol/collect-flash-fees`, receives a base64-encoded compiled transaction, signs it with the user's wallet, and broadcasts it on Solana devnet.
...
```bash
npx ts-node src/scripts/solana/claim-flash-token-fees.ts
```

Or build and run:

```bash
npm run build && node dist/scripts/solana/claim-flash-token-fees.js
```

## Parameters

The `claimFlashTokenFeesSolana` function accepts a `ClaimFeesSolanaRequest` type:
...
//   address: solanaAddressSchema,    // Flash token mint address
// })
```
...
| `address` | `string` | Flash token mint address    | Valid Solana base58 address |
...
3. **API Request** - Payload sent to `https://cdn.based.bid/api/sol/collect-flash-fees`:
   ```typescript
   {
...
```typescript
import { claimFlashTokenFeesSolana } from './src/scripts/solana/claim-flash-token-fees';
...
| `Failed to claim flash token fees on Solana` | API error | Check token address and API availability |
| `Transaction failed` | On-chain failure | Verify you are the token owner, ensure fees exist |

## Differences from LBP Fee Collection

| Aspect | LBP Fees | Flash Token Fees |
|--------|----------|------------------|
| API endpoint | `/sol/collect-lbp-fees` | `/sol/collect-flash-fees` |
